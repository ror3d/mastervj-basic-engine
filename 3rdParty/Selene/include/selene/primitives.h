...
template <typename T>
struct is_primitive {
	static _constexpr bool value = false;
};
template <>
...
inline std::string _get(_id<const std::string&>, lua_State *l, const int index) {
	size_t size;
...
inline lua_Number _check_get(_id<double>, lua_State *l, const int index) {
	int isNum = 0;
...
inline lua_Number _check_get(_id<float>, lua_State *l, const int index) {
	int isNum = 0;
	auto res = lua_tonumberx(l, index, &isNum);
	if(!isNum){
		throw GetParameterFromLuaTypeError{
...
inline void _push(lua_State *l, const std::tuple<T...> &values) {
	_constexpr int num_values = sizeof...(T);
