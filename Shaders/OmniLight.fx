...
float4 PS(PS_INPUT IN) : SV_Target
{
	...
	float Atten = clamp((m_LightAttenuationEndRangeArray[0]-dist) * (1/(m_LightAttenuationEndRangeArray[0]-m_LightAttenuationStartRange[0])), 0, 1);
	
	//calculos difuso
	float l_DiffuseContrib = max(0, dot(Nn, L)) * m_LightIntensityArray[0] * Atten;
	...
	return float4((m_LightAmbient.xyz * l_Albedo.xyz + l_DiffuseContrib * l_Albedo.xyz * m_LightColor[0] + Atten * l_SpecularContrib * m_LightColor[0]), l_Albedo.a);
}